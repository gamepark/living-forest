...
import { HistoryEntry, MaterialLogProps, usePlayerName } from '@gamepark/react-game'
import { isMoveItemType, MaterialMove, StartRule } from '@gamepark/rules-api'
import { FC } from 'react'
import { useTranslation } from 'react-i18next'
import { getColor } from '../../utils/ColorUtils'

type OnibiAttacksPlayerRuleHistoryProps = { move: StartRule } & Omit<MaterialLogProps, 'move'>

export const OnibiAttacksPlayerRuleHistory: FC<OnibiAttacksPlayerRuleHistoryProps> = (props) => {
  const { t } = useTranslation()
  const { move, context } = props
  const { game, action } = context
  const varans = action.consequences.filter((move: MaterialMove) =>
